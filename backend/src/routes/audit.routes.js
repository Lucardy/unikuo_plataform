 ...
 */
router.get('/statistics', authenticate, requireRole(['admin']), auditController.getAuditStatistics);

/**
 * GET /api/audit/:id
 * Obtener registro de auditoría por ID (requiere autenticación y rol admin)
 */
router.get('/:id', authenticate, requireRole(['admin']), auditController.getAuditLogById);
